...
* **模型架构：** CLIP (ViT-B/32)
* **数据集：** CIFAR-10 Test Set (32x32 pixel resolution)
* **计算环境：** Python 3.x, PyTorch, CUDA (Single GPU)
* **核心算法：** 向量正交切除 (Vector Orthogonal Subtraction)
...
```
![实验结果可视化](stage2可视化.png)

# 最终实验总结以及结果解释

## 总结：
首先我们定位了猫的特征向量位置，然后我们编写了手术函数，紧接着算出最佳手术强度Alpha便开始推理。
随后我们进行了消融实验，得到结果并进行了可视化。

### 核心指标对比

| 类别 (Class) | 基线准确率 (Baseline) | 术后准确率 (Ours) | 变化 () | 评价 |
| --- | --- | --- | --- | --- |
| **Cat** | 87.8% | **60.1%** | **-27.7%** | 🎯 **精准达标** |
| **Dog** | 90.7% | **93.0%** | +2.3% | 📈 正向副作用 |
| **Bird** | 93.2% | **97.5%** | +4.3% | 📈 正向副作用 |

> **图表说明：** 上图A展示了干预前后的性能对比。可以看到 Cat 被精准压制，而 Dog 和 Bird 的识别率反而上升。这是由于 Softmax 的归一化特性：当 Cat 的 Logits 被压低后，概率空间被重新分配给了与其正交的其他高置信度类别。

## 结果解释

### 主要问题：为什么只切除了猫而不伤害鸟和狗，为什么鸟和狗的概率反而上升

#### 原因1(为什么不伤害)：权重分布的正交性
我们可以把三个动物的特征摆在X,Y,Z三个轴上面。比如说猫的特征在x轴上，狗在y轴，鸟在z轴。我们随机得到一个坐标(10,10,10)
现在我们改猫的权重，比如说那么猫的值就会在x轴上被拉扯，但是因为这三个动物的向量是互相垂直的，所以x轴乘上权重后没有任何
投影会投在y，z轴上面，所以就不会改变猫狗的值。假设最后得到猫的值由10变为2，那么最后出来的坐标就是(2,10,10)

#### 原因2(为什么上升)：softmax的归一化
softmax函数让猫狗鸟把1给分完了。现在猫分到的少了，那么最后狗和鸟就自然而然地分多了

### 消融实验 

为了验证方法的科学性与鲁棒性，我们进行了两组高阶消融实验。

#### 梯度响应分析

* **实验设置：** 将干预通道比例从 0% 线性增加至 100%。
* **观察结果：** 准确率随干预比例呈现高度线性的下降趋势 ()。
* **结论：** 证明了该特征手术方法具有极高的**可控性 (Controllability)**。模型性能不是“非黑即白”的突变，而是可以通过参数精细调节的连续系统。

#### 特征稀疏性验证

* **实验设置：** 在固定 50% 干预比例下，对比不同通道的重要性。
* **Top-50% (High Activation):** 仅切除激活值最高的通道。
* **Bottom-50% (Low Activation):** 仅切除激活值最低的通道。


* **数据对比：**
* Top-50% 准确率：**64.0%** (效果显著，接近全量干预)
* Bottom-50% 准确率：**85.0%** (效果微弱，接近无干预)


* **结论：** 实验强有力地证明了 CLIP 特征空间的**稀疏性 (Sparsity)**。“猫”的语义高度集中在头部的特定神经元中，而非均匀分布。这意味着我们只需修改极少量的神经元即可实现高效控制。

实验成功实现了对 CLIP 模型的**定向概念擦除 (Concept Erasure)**。

1.  成功将 Cat 类别准确率控制在 60.1% (误差 < 0.1%)。
2.  实现了“手术刀”操作，未对其他类别造成负面影响，反而因概率重分配提升了 Dog/Bird 的表现。
3.  通过消融实验揭示了特征向量的稀疏分布规律，为后续的“最小化干预”或“多概念并发控制”奠定了理论基础。
